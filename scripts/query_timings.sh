...
pqt=$($PSQL -h $PG_HOST -U $PG_USER -d $PG_DB -p $PG_PORT --no-psqlrc -q -t -A -c "select pid as PID,datname as DATABASE,usename as USER,client_addr as HOST,now() - query_start as DURATION,state as STATE,query as QUERY from pg_stat_activity where state='active' order by duration desc")
echo "--------------------------------------"
